            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                <div class="my-4">
                    ...
                    <form action="update.php" method="post">
                        <p class="form-group">
                            <label for="data_checkin">Data_Checkin</label>
                            <input type="text" name="data_checkin" class="form-control" value="<?= $reserva->getData_Checkin() ?>">
                        </p>
                        <p class="form-group">
                            <label for="data_checkout">Data_Checkout</label>
                            <input type="text" name="data_checkout" class="form-control" value="<?= $reserva->getData_Checkout() ?>">
                        </p>
                        <p class="form-group">
                            <label for="valor_total">Valor_Total</label>
                            <input type="text" name="valor_total" class="form-control" value="<?= $reserva->getValor_Total() ?>">
                        </p>
                    
                        <!-- Para a chave estrangeira (associação com funcionario) -->
                        <p class="form-group">
                            <label for="funcionario">Tipo de Funcionario</label>
                            <select name="funcionario" class="form-control">
                                <?php foreach($daoFuncionario->read() as $funcionario) : ?>
                                    <?php if($reserva->getFuncionario()->getId() == $funcionario->getId()) : ?>
                                        <option selected value="<?= $funcionario->getId() ?>"><?= $funcionario->getNome() ?></option>
                                    <?php else : ?>
                        ...
                        <p class="form-group">
                            <label for="hospede">Tipo de Hospede</label>
                            <select name="hospede" class="form-control">
                                <?php foreach($daoHospede->read() as $hospede) : ?>
                                    <?php if($reserva->getHospede()->getId() == $hospede->getId()) : ?>
                                        <option selected value="<?= $hospede->getId() ?>"><?= $hospede->getNome() ?></option>
                                    <?php else : ?>
                                        <option value="<?= $hospede->getId() ?>"><?= $hospede->getNome() ?></option>
                                    <?php endif ?>
                                <?php endforeach ?>
                            </select>
                        </p>
                        <p class="form-group">
                            ...
                            <select name="status_reserva" class="form-control">
                                <?php foreach($daoStatusReserva->read() as $status_reserva) : ?>
                                    <?php if($reserva->getStatus_Reserva()->getIdStatusReserva() == $status_reserva->getIdStatusReserva()) : ?>
                                        <option selected value="<?= $status_reserva->getIdStatusReserva() ?>"><?= $status_reserva->getDescricao() ?></option>
                                    <?php else : ?>
                                        <option value="<?= $status_reserva->getIdStatusReserva() ?>"><?= $status_reserva->getDescricao() ?></option>
                                    <?php endif ?>
                                <?php endforeach ?>
                            </select>
                        </p>
